...
class PostgreSQLSettings(BaseSettings):
    ...
    database_url: str = Field(default="postgresql://rag_user:rag_password@localhost:5432/rag_db", description="Database connection string url")
    echo_sql: bool = Field(default=False, description="Enable SQL Query Logging")
    pool_size: int = Field(default=20, description="Pool size for database connections")
    max_overflow: int = Field(default=0, description="Maximum pool overflow for database connections")
...
class PostgreSQLDatabase(BaseDatabase):
    ...
    def startup(self) -> None:
        ...
        try:
            ...
            # create tables if they dont exist (idempotent operation)
    ...
    def get_session(self) -> Generator[Session, None, None]:
        ...
        if not self.session_factory:
            raise RuntimeError("Database session not initialized call startup() first")
